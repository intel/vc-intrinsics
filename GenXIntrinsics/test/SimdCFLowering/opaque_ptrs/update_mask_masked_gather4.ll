...
define dso_local dllexport void @test(<32 x i16> %cond1, <32 x i16> %cond2, <32 x i32> %addrs, <32 x i1> %pred) {
  ...
  %cmp1 = icmp ne <32 x i16> %cond1, zeroinitializer
  %call = call i1 @llvm.genx.simdcf.any.v32i1(<32 x i1> %cmp1)
...
; CHECK-NEXT:  [[CALL1_SIMDCFPRED1:%.*]] = select <64 x i1> [[CHENNELEM]], <64 x i32> [[CALL1]]
  store <64 x i32> %call1, ptr %g
  %cmp2 = icmp ne <32 x i16> %cond2, zeroinitializer
  %nest = call i1 @llvm.genx.simdcf.any.v32i1(<32 x i1> %cmp2)
  br i1 %nest, label %if.then2, label %if.end2

; CHECK-LABEL: if.then2:
if.then2:
; CHECK: [[EM_LOAD3:%.*]] = load <32 x i1>, ptr @EM
...
if.end:
  %ld = load <64 x i32>, ptr %g
  store <64 x i32> %ld, ptr @g1
  ret void
}
